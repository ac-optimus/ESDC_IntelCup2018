...
#the code is modification of example code of Intel Movdius ncsappzoo
...
def overlay_on_image(display_image, object_info):
    ...
    #put the threshold confidence score here 
    ...
    if (percentage <= min_score_percent):
        ...
        return
    if (LABELS[int(class_id)] == "person"): #draw boxes only for person 
...
def contineous_video():
 #   start = time.time()
    ...
    cv_window_name = "PersonDetection using SSD-MObilenet"

    devices = mvnc.EnumerateDevices() #create an instance of the device
    if len(devices) == 0:
        print('No movidus found')
        quit()

    #start = time.time()
    device = mvnc.Device(devices[0]) #devices contains list of all the movidus found
   ...
   # graph_file_name = 'graph'
    ...
    start = time.time()
    for i in range(60):
   # while (True):#uncomment to not calculate fps
      ret, frame = vid.read()
	   # forward pass(inference)
      run_inference(frame, graph)
    # display the results and wait for user to hit a key
      cv2.imshow("PersonDetection using SSD-MObilenet", frame)
...
def single_image():
   ...
   # graph_file_name = 'graph'
    ...
    #dehazing
    image = AOD.AOD_Net_modify_image(image)
    #make a copy if this does not work
	   # forward pass(inference)
    start = time.time()
    run_inference(image, graph)
    end = time.time()
    print "inference time =",(end - start)
    # display the results and wait for user to hit a key

    cv2.imshow("PersonDetection using SSD-MObilenet", image)
    cv2.waitKey(0) 
    ...
    graph.DeallocateGraph()
    device.CloseDevice()
    #uncomment the below to return the image as a matrix
    #return image 
